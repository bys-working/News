...
namespace Application\BjyAuthorize\Provider\Role;
...
class ZendDb implements ProviderInterface
{
    ...
    public function getRoles()
    {
        ...
        if ($this->serviceLocator->get('zfcuser_auth_service')->getIdentity()) {
            $username = $this->serviceLocator->get('zfcuser_auth_service')->getIdentity()->getUsername();
        } else {
            $username = null;
        }
        ...
        foreach ($rowset as $row) {
            ...
            if (($username != null) && ($row['is_default'] == 1)) {
                $row[$this->parentRoleFieldName] = $username;
            }
            $roles[$roleId] = new Role($roleId, $row[$this->parentRoleFieldName]);
        }

        if ($username) {
            $roles[$username] = new Role($username);
        }
